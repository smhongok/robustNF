...
class LRHR_IMGDataset_nocrop(data.Dataset):
    def __init__(self, opt, divide_freq):
        super(LRHR_IMGDataset_nocrop, self).__init__()
        self.opt = opt
        self.crop_size = opt.get("GT_size", None)
        self.scale = opt['scale']
        self.random_scale_list = [1]
        self.divide_freq = divide_freq
    ...
    def __getitem__(self, item):
        ...
        if self.divide_freq:
            lr_up = upsample(lr, scale=self.scale)
            hr = hr - lr_up
...
def upsample(lr, scale=4):
    lr = lr.unsqueeze(0)
    return torch.nn.functional.upsample(lr, scale_factor=scale, mode='bicubic').squeeze(0)
