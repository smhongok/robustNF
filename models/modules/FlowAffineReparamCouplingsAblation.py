...
class CondAffineReparamSeparatedAndCond(nn.Module):
    def __init__(self, in_channels, opt, std_mode=True):
        ...
        self.fAffine_std = self.F(in_channels=self.channels_for_nn + self.std_channels,
                              out_channels=self.channels_for_co * 2,
                              hidden_channels=self.hidden_channels,
                              kernel_hidden=self.kernel_hidden,
                              n_hidden_layers=self.n_hidden_layers)
    ...
    def feature_extract(self, z, f):
        ...
        scale = 1./(torch.sigmoid(scale + 2.) + self.affine_eps)
        return scale, shift
