...
class CondRQsplineSeparatedAndCond(nn.Module):
    def __init__(self, in_channels, opt, std_mode=True, bias=True):
        ...
        self.fFeatures = self.F(in_channels=self.in_channels_rrdb,
                                out_channels=self.in_channels * (3+self.bias),
                                # 2 for (shift, scale)
                                hidden_channels=self.hidden_channels,
                                kernel_hidden=self.kernel_hidden,
                                n_hidden_layers=self.n_hidden_layers)
        
        self.fRQspline_std = self.F(in_channels=self.channels_for_nn + self.std_channels,
                               out_channels=self.channels_for_co * (3+self.bias),
                               # 2 for (shift, scale)
                               hidden_channels=self.hidden_channels,
                               kernel_hidden=self.kernel_hidden,
        ...
        self._left = opt_get(opt, ['network_G', 'flow', 'CondRQsplineSeparatedAndCond', 'left'], -0.5)
        self._right = opt_get(opt, ['network_G', 'flow', 'CondRQsplineSeparatedAndCond', 'righ'], 0.5)
        self._top = opt_get(opt, ['network_G', 'flow', 'CondRQsplineSeparatedAndCond', 'top'], 0.5)
        self._bottom = opt_get(opt, ['network_G', 'flow', 'CondRQsplineSeparatedAndCond', 'bottom'], -0.5)
    ...
    def forward(self, input: torch.Tensor, logdet=None, reverse=False, ft=None, std=None):
        ...
        if not reverse:
            ...
            widthFt, heightFt, derivativeFt, shiftFt = self.feature_extract(ft, self.fFeatures)
            z, logdetFt = rational_quadratic_spline(
                z,
                widthFt,
                heightFt,
                derivativeFt,
            ...
            if self.stdmode and std is not None:
                ...
                widthFt, heightFt, derivativeFt, shiftFt = self.feature_extract_RQspline(z1, std_expand, self.fRQspline_std)
...
def searchsorted2(bin_locations, inputs):
    return (inputs >= bin_locations[..., 1]).to(torch.int64)
...
def rational_quadratic_spline(
    ...
    if inverse:
        bin_idx = searchsorted2(cumheights, inputs)[..., None]
    else:
        bin_idx = searchsorted2(cumwidths, inputs)[..., None]
    #print(cumwidths)
    #print(bin_idx)
