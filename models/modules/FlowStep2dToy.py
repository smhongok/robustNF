...
from models.modules import flow, thops, FlowAffineCouplingsAblation2dToy, FlowBsplineCouplingsAblation2dToy,  FlowRQsplineCouplingsAblation2dToy, StdConditional
...
class FlowStep2dToy(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels,
                 actnorm_scale=1.0, flow_permutation="invconv", flow_coupling="CondAffineSeparatedAndCond2dToy", stdCondition=True,
                 LU_decomposed=False, opt=None, image_injector=None, idx=None, acOpt=None, normOpt=None, in_shape=None,
                 position=None):
        # check configures
        assert flow_permutation in FlowStep2dToy.FlowPermutation, \
            "float_permutation should be in `{}`".format(
                FlowStep2dToy.FlowPermutation.keys())
        super().__init__()
        ...
        # 3. coupling
        if flow_coupling == "CondAffineSeparatedAndCond2dToy":
            self.affine = models.modules.FlowAffineCouplingsAblation2dToy.CondAffineSeparatedAndCond2dToy(in_channels=in_channels,
                                                                                                opt=opt,
                                                                                                std_mode=self.std_mode)
        elif flow_coupling == "CondBsplineSeparatedAndCond2dToy":
            self.bspline = models.modules.FlowBsplineCouplingsAblation2dToy.CondBsplineSeparatedAndCond2dToy(
                in_channels=in_channels,
                opt=opt, std_mode=self.std_mode)
        elif flow_coupling == "CondRQsplineSeparatedAndCond2dToy":
            self.rqspline = models.modules.FlowRQsplineCouplingsAblation2dToy.CondRQsplineSeparatedAndCond2dToy(
                in_channels=in_channels,
                opt=opt, std_mode=self.std_mode)
    ...
    def normal_flow(self, z, logdet, rrdbResults=None, std=None):
        ...
        # 2. permute
        z, logdet = FlowStep2dToy.FlowPermutation[self.flow_permutation](
             self, z, logdet, False)
        ...
        elif self.flow_coupling == "CondBsplineSeparatedAndCond2dToy":
            img_ft = getConditional(rrdbResults, self.position)
            z, logdet = self.bspline(input=z, logdet=logdet, reverse=False, ft=img_ft, std=std)
        elif self.flow_coupling == "CondRQsplineSeparatedAndCond2dToy":
            img_ft = getConditional(rrdbResults, self.position)
    ...
    def reverse_flow(self, z, logdet, rrdbResults=None, std=None):
        ...
        print(torch.var(z, dim=0)[0, 0].detach().numpy()[0], ",", torch.mean(z, dim=0)[0, 0].detach().numpy()[0],
              ",", torch.var(z, dim=0)[1, 0].detach().numpy()[0], ",", torch.mean(z, dim=0)[1, 0].detach().numpy()[0])
        # 2. permute
        z, logdet = FlowStep2dToy.FlowPermutation[self.flow_permutation](
            self, z, logdet, True)
        print(torch.var(z, dim=0)[0, 0].detach().numpy()[0], ",", torch.mean(z, dim=0)[0, 0].detach().numpy()[0],
