...
def main():
    ...
    print(args.opt)
    opt = option.parse(args.opt, keys, values, is_train=True)
    opt['_resume'] = args.resume_ckpt
    print("opt")
    print(opt)
    ...
    if opt['path'].get('resume_state', None):
        resume_state_path, _ = get_resume_paths(opt, opt['_resume'])
    ...
    with open(os.path.join(opt['path']['log'], 'config.yml'), 'w') as outfile:
        yaml.dump(opt, outfile)
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            train_set = create_dataset(dataset_opt, opt['divide_freq'])
            ...
            val_set = create_dataset(dataset_opt, opt['divide_freq'])
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                for val_data in tqdm(val_loader):
                    ...
                    model.feed_data(val_data, val_mode=True)
